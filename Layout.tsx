    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-200 font-sans">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-gray-700 shadow-soft z-20 transition-colors">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 ${
                currentView === item.id || (currentView === 'chat' && item.id === 'form')
                  ? 'bg-medical-50 text-medical-600 shadow-sm dark:bg-medical-900/30 dark:text-medical-400'
                  : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-slate-700 dark:hover:text-gray-200'
              }`}
            >
              {item.icon}
              {item.label}
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-700">
          ...
          <div 
             className="flex items-center gap-3 px-3 py-3 rounded-xl bg-gray-50 dark:bg-slate-700 mb-2 transition-colors cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-600"
             onClick={() => onNavigate('settings')}
          >
             <div className="w-8 h-8 rounded-full bg-medical-100 dark:bg-medical-900 text-medical-600 dark:text-medical-300 flex items-center justify-center font-bold text-xs">
                {userEmail?.charAt(0).toUpperCase()}
             </div>
             <div className="flex-1 min-w-0">
               <p className="text-xs font-semibold text-gray-900 dark:text-gray-200 truncate">{userEmail}</p>
               <p className="text-[10px] text-green-600 dark:text-green-400 font-medium">Pro Plan Active</p>
             </div>
          </div>
          <button 
            onClick={onSignOut} 
            className="w-full flex items-center justify-center gap-2 text-xs font-medium text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20 py-2 rounded-lg transition-colors"
          >
            Sign Out
          </button>
        </div>
      ...
      <main className="flex-1 relative flex flex-col h-full overflow-hidden bg-slate-50 dark:bg-slate-900 transition-colors">
        ...
        <header className="md:hidden h-16 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 sticky top-0 z-20 transition-colors">
           ...
           <div className="flex items-center gap-2">
             <DarkModeToggle />
             <button onClick={onSignOut} className="text-gray-500 dark:text-gray-400">
               <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
        ...
        {/* Mobile Bottom Nav */}
        <nav className="md:hidden h-16 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-gray-700 flex justify-around items-center px-4 pb-safe transition-colors">
           {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => onNavigate(item.id as ViewState)}
